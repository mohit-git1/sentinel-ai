# Changelog

All notable changes to Sentinel AI will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/).

## [1.0.0] - 2026-03-05

### Added
- GitHub OAuth authentication with JWT session management
- Webhook listener for pull request events (opened, synchronize)
- AI-powered code review using OpenAI GPT-4o-mini
- Inline GitHub comment posting via Octokit
- Repository management — connect/disconnect repos with one click
- Dashboard with PR review history and analytics
- MongoDB models for Users, Repositories, Pull Requests, and Reviews
- React frontend with Tailwind CSS
- Comprehensive API routes for auth, repos, webhooks, and reviews
- Documentation: setup guide, API reference, architecture overview
- MIT License and contributing guidelines
